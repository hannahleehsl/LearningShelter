...
const Courses = ({ lessons }) => {
  ...
  return (
    <div style={{ backgroundImage: `url(${image})` }}>
      ...
      <div style={{ display: "flex", flexDirection: "row", paddingTop: "3rem" }}></div>
      <h3 style={{ textAlign: "center", marginBottom: "40px" }}>Current Lesson: {currentLesson?.title}</h3>
        ...
        <div>
          ...
          {currentLesson?.videos.map((video) => (
            <div key={video.id} style={{ textAlign: "center", marginBottom: "5rem" }}>
              <h3 style={{fontSize: "25px", fontFamily: "Mono", marginBottom: "40px" }}>{video.title}</h3>
              <iframe
                width="560"
                height="315"
